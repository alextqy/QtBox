...
class MyDepartmentFrame(BaseInterface, BaseFrame):
    ...
    def __init__(self, DepartmentID):
        ...
        self.UserHeaderLabel.setStyleSheet(
            self.Style.Object.MainFrame_Mid_My_Department_User_Header())  # 设置样式
        ...
        self.UserTree.setStyleSheet(
            self.Style.Object.MainFrame_Mid_My_Department_User_Tree_Widget())  # 设置样式
        ...
        self.FileHeaderLabel.setStyleSheet(
            self.Style.Object.MainFrame_Mid_My_Department_File_Header())  # 设置样式

        self.FileTree = BaseTreeWidget()
        self.FileTree.SetSelectionMode(2)  # 设置多选模式
        self.FileTree.setStyleSheet(
            self.Style.Object.MainFrame_Mid_My_Department_File_Tree_Widget())  # 设置样式
        ...
        self.FileTree.setHeaderLabels(
            ["FILE", "FileID", "CreateTime", "ID"])  # 设置标题栏
    ...
    def InsertFileListData(self, UserID=0):
        ...
        if len(self.FileList) > 0:
            for i in range(len(self.FileList)):
                ...
                item.setText(
                    0, FileInfo["FileName"])  # 设置内容
                item.setText(
                    1, str(FileInfo["ID"]))  # 设置内容
                item.setText(
                    2, str(self.Common.TimeToStr(self.FileList[i]["Createtime"])))  # 设置内容
                item.setText(
                    3, str(self.FileList[i]["ID"]))  # 设置内容
    ...
    def UserRightContextMenuExec(self, pos):
        ...
        self.UserTreeMenu.setStyleSheet(
            self.Style.Object.MainFrame_Mid_My_Department_User_Tree_Menu())  # 设置样式
        ...
        if type(Item) == QTreeWidgetItem and type(ItemAt) == QTreeWidgetItem:  # 焦点内
            ...
            self.UserTreeMenu.AddAction(
                self.Lang.Refresh, lambda: self.InsertUserListData())
    ...
    def FileRightContextMenuExec(self, pos):
        ...
        if type(Item) == QTreeWidgetItem and type(ItemAt) == QTreeWidgetItem:  # 焦点内
            self.FileTreeMenu.AddAction(
                self.Lang.Download, lambda: self.Download())
            self.FileTreeMenu.AddAction(
                self.Lang.Remove, lambda: self.RemoveFile())
        else:  # 焦点外
            self.FileTreeMenu.AddAction(
                self.Lang.Refresh, lambda: self.InsertFileListData())
