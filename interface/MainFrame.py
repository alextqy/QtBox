...
class MainFrame(BaseInterface, BaseFrame):
    ...
    def __init__(self):
        ...
        self.TaskUploadBtn.setStyleSheet(
            self.Style.Object.MainFrame_Top_Task_Btn())  # 设置样式
        self.TaskUploadBtn.clicked.connect(
            lambda: self.UploadListWindowObject.show())  # 连接槽函数
        ...
        self.TaskDownLoadBtn.clicked.connect(
            lambda: self.DownloadListWindowObject.show())  # 连接槽函数
        ...
        self.MidLayoutLFrame.setStyleSheet(
            self.Style.Object.MainFrame_Mid_Frame_L())  # 设置样式
        ...
        if self.SelfData["Account"] != "root":
            if self.SelfData["DepartmentID"] > 0:
                self.MyDepartmentLabel = MyDepartmentLabel(
                    self.Lang.MyDepartment)  # 我的部门
                self.MyDepartmentLabel.setFixedSize(
                    MidLQtWidgetsWidth, MidLQtWidgetsHeight)  # 设置固定大小
                self.MyDepartmentLabel.setAlignment(Qt.AlignCenter)  # 字体居中
                self.MyDepartmentLabel.setStyleSheet(
                    self.Style.Object.MainFrame_Mid_Banner_Btn())  # 设置样式
                if self.SelfData["DepartmentID"] > 0:
        ...
        if self.IsMaster == True:
            ...
            self.UserLabel.setFixedSize(
                MidLQtWidgetsWidth, MidLQtWidgetsHeight)  # 设置固定大小
            self.UserLabel.setAlignment(Qt.AlignCenter)  # 字体居中
            self.UserLabel.setStyleSheet(
                self.Style.Object.MainFrame_Mid_Banner_Btn())  # 设置样式
            self.UserLabel.ActionSignal.connect(
                lambda: self.ShowControl("UserFrame"))  # 连接槽函数
            self.MidLV.addWidget(self.UserLabel)  # 添加到布局

            self.DepartmentLabel = DepartmentLabel(
                self.Lang.DepartmentManagement)  # 部门管理
            self.DepartmentLabel.setFixedSize(
                MidLQtWidgetsWidth, MidLQtWidgetsHeight)  # 设置固定大小
            self.DepartmentLabel.setAlignment(Qt.AlignCenter)  # 字体居中
            self.DepartmentLabel.setStyleSheet(
                self.Style.Object.MainFrame_Mid_Banner_Btn())  # 设置样式
            self.DepartmentLabel.ActionSignal.connect(
                lambda: self.ShowControl("DepartmentFrame"))  # 连接槽函数
            self.MidLV.addWidget(self.DepartmentLabel)  # 添加到布局

            self.DirFileLabel = DirFileLabel(
                self.Lang.FileManagement)  # 文件 文件夹
            self.DirFileLabel.setFixedSize(
                MidLQtWidgetsWidth, MidLQtWidgetsHeight)  # 设置固定大小
            self.DirFileLabel.setAlignment(Qt.AlignCenter)  # 字体居中
            self.DirFileLabel.setStyleSheet(
                self.Style.Object.MainFrame_Mid_Banner_Btn())  # 设置样式
            self.DirFileLabel.ActionSignal.connect(
                lambda: self.ShowControl("DirFileFrame"))  # 连接槽函数
            ...
            self.SysConfLabel.setFixedSize(
                MidLQtWidgetsWidth, MidLQtWidgetsHeight)  # 设置固定大小
            self.SysConfLabel.setAlignment(Qt.AlignCenter)  # 字体居中
            self.SysConfLabel.setStyleSheet(
                self.Style.Object.MainFrame_Mid_Banner_Btn())  # 设置样式
            self.SysConfLabel.ActionSignal.connect(
                lambda: self.ShowControl("SysConfFrame"))  # 连接槽函数
        ...
        self.MidLayoutRFrame.setStyleSheet(
            self.Style.Object.MainFrame_Mid_Frame_R())  # 设置样式
...
class CheckMyself(BaseInterface, BaseDialog):
    ...
    def __init__(self, UserData):
        ...
        self.TextLabel.setStyleSheet(
            self.Style.Object.MainFrame_CheckMyself_Avatar_Label())  # 设置样式
        ...
        self.AccountInput.setStyleSheet(
            self.Style.Object.MainFrame_CheckMyself_Input())  # 设置样式
        ...
        if self.UserData["Avatar"] != "":
            AvatarByte = QByteArray.fromBase64(
                self.UserData["Avatar"].encode("utf8"))  # 读取二进制信息
